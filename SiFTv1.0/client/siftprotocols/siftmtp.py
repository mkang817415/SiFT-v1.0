...
class SiFT_MTP:
	...
	def send_msg(self, msg_type, msg_payload):
		if msg_type == self.type_login_req: # Message is login
			...
			#########
			# etK (Encrypted Temporary Key)
			# -> tmp key 32 bytes AES key -> produces encrypted payload and authentication tag (mac) of the message using AES in GCM mode. 
			temp_key = Random.get_random_bytes(32) # 256 bits, 32 bytes AES key
			nonce = sqn + rnd
			AES_GCM = AES.new(temp_key, AES.MODE_GCM, nonce=nonce,mac_len=16)
			AES_GCM.update(msg_payload)
			epd, mac = AES_GCM.encrypt_and_digest(msg_payload) # epd = encrypted payload, mac = authentication tag

			# temporary key is encrypted using RSA-OAEP with a 2048-bit RSA public key 
			...
			# nonce
			nonce = sqn + rnd
			key = self.transfer_key # final transfer key

   
			AES_GCM = AES.new(key, AES.MODE_GCM, nonce=nonce,mac_len=16)
			AES_GCM.update(msg_payload)
			epd, mac = AES_GCM.encrypt_and_digest(msg_payload) # epd = encrypted payload, mac = authentication tag
